<script>
  ...
  const { description = "...", price = "$ 0.00", crewmate = [] } = provision;
  const handleSelect = () => {
    const remove =
...
<div on:click={handleSelect} class="relative h-32 w-32">
  <svg
  ...
  <div class="absolute w-32 h-32 flex flex-col justify-between text-center">
    ...
    <dd>
      <div
        contenteditable
        class="border-4 border-gray-900 bg-green-500 text-white center-self text-xl font-medium bg-orange-500 rounded-lg">
        ${price}
      </div>
    </dd>
    <dt contenteditable class="text-md font-medium text-black truncate">
      {description}
    </dt>
  </div>
</div>
